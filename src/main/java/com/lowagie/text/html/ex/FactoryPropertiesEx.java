 ...
 */
    ...
    /**
     * THIS FILE WAS MODIED.
     * CHANGE MARKAUP CLASS AND REFERENSE.
     */
...
public class FactoryPropertiesEx {
    ...
    /**
     * This method isn't used by iText, but you can use it to analyze the value of a style attribute inside a HashMap. The different
     * elements of the style attribute are added to the HashMap as key-value pairs.
     * 
     * @param h a HashMap that should have at least a key named style. After this method is invoked, more keys could be added.
     */
    public static void insertStyle(HashMap h) {
        String style = (String) h.get("style");
        if (style == null)
            return;
        Properties prop = parseAttributes(style);
        for (Iterator it = prop.keySet().iterator(); it.hasNext();) {
            String key = (String) it.next();
            if (key.equals(CSS_KEY_FONTFAMILY)) {
                h.put("face", prop.getProperty(key));
            } else if (key.equals(CSS_KEY_FONTSIZE)) {
                h.put("size", Float.toString(parseLength(prop.getProperty(key))) + "pt");
            } else if (key.equals(CSS_KEY_FONTSTYLE)) {
                String ss = prop.getProperty(key).trim().toLowerCase();
                if (ss.equals("italic") || ss.equals("oblique"))
                    h.put("i", null);
            } else if (key.equals(CSS_KEY_FONTWEIGHT)) {
                String ss = prop.getProperty(key).trim().toLowerCase();
                if (ss.equals("bold") || ss.equals("700") || ss.equals("800") || ss.equals("900"))
                    h.put("b", null);
            } else if (key.equals(CSS_KEY_TEXTDECORATION)) {
                String ss = prop.getProperty(key).trim().toLowerCase();
                if (ss.equals(CSS_VALUE_UNDERLINE))
                    h.put("u", null);
            } else if (key.equals(CSS_KEY_COLOR)) {
                Color c = decodeColor(prop.getProperty(key));
                if (c != null) {
                    int hh = c.getRGB();
            ...
            } else if (key.equals(CSS_KEY_LINEHEIGHT)) {
            ...
            } else if (key.equals(CSS_KEY_TEXTALIGN)) {
                String ss = prop.getProperty(key).trim().toLowerCase();
                h.put("align", ss);
            }
        }
    ...
    public static void insertStyle(HashMap h, ChainedProperties cprops) {
        ...
        for (Iterator it = prop.keySet().iterator(); it.hasNext();) {
            ...
            if (key.equals(CSS_KEY_FONTFAMILY)) {
                ...
                h.put(ElementTags.SIZE, Float.toString(parseLength(prop.getProperty(key), actualFontSize)) + "pt");
            } else if (key.equals(CSS_KEY_FONTSTYLE)) {
                String ss = prop.getProperty(key).trim().toLowerCase();
                if (ss.equals("italic") || ss.equals("oblique"))
                    h.put("i", null);
                ...
                String ss = prop.getProperty(key).trim().toLowerCase();
                h.put("align", ss);
            } else if (key.equals(CSS_KEY_PADDINGLEFT)) {
                String ss = prop.getProperty(key).trim().toLowerCase();
